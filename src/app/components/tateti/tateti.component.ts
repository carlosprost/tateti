...
export class TatetiComponent {
  ...
  ia() {
    let aiEspera = setInterval(() => {
      if (!this.X) {
        setTimeout(() => {
          this.comprobarJugadaGanadora();
          let casillas = document.querySelectorAll('.casilla');
          let casillaVacia = Array.from(casillas).filter(
            (v) => v.innerHTML == ''
          );
          let casilla = casillaVacia[
            Math.floor(Math.random() * casillaVacia.length)
          ] as HTMLDivElement;
  ...
  esColumna1(casillas: Element[]) {
    return (
      casillas[0].id == '0' && casillas[1].id == '3' && casillas[2].id == '6'
    );
  }

  esColumna2(casillas: Element[]) {
    return (
      casillas[0].id == '1' && casillas[1].id == '4' && casillas[2].id == '7'
    );
  }

  esColumna3(casillas: Element[]) {
    return (
      casillas[0].id == '2' && casillas[1].id == '5' && casillas[2].id == '8'
    );
  }

  esDiagonal1(casillas: Element[]) {
    return (
      casillas[0].id == '2' && casillas[1].id == '4' && casillas[2].id == '6'
    );
  }

  esDiagonal2(casillas: Element[]) {
    return (
      casillas[0].id == '0' && casillas[1].id == '4' && casillas[2].id == '8'
    );
  }

  esFila1(casillas: Element[]) {
    return (
      casillas[0].id == '0' && casillas[1].id == '1' && casillas[2].id == '2'
    );
  }

  esFila2(casillas: Element[]) {
    return (
      casillas[0].id == '3' && casillas[1].id == '4' && casillas[2].id == '5'
    );
  }

  esFila3(casillas: Element[]) {
    return (
      casillas[0].id == '6' && casillas[1].id == '7' && casillas[2].id == '8'
    );
  }
